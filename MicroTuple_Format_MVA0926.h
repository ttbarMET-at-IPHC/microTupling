...
//                       #            Keep me updated !             #
//                       ############################################
#define MICROEVENT_FORMATROOT "mStop:mNeutralino:nMCLepton:nJets:nBTag:MT:MET:MT2W:dPhiMETjet:HTratio:HadronicChi2:leadingBPt:leptonPt:trueNumberOfEvents:nWTag/I:nWTag2"
...
Bool_t MicroTuple_ProofJob::Process(Long64_t entry)
{
  ...
  // ######################
    ...
    for (unsigned int i = 0 ; i < WCand.size() ; i++)
    {
        ...
        // Subjetiness ratio
        if (WCand[i].others["tau2"] / WCand[i].others["tau1"] < 0.5)
            myEvent.nWTag += 1.0;
