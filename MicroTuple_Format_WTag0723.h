...
typedef struct 
{
    ...
    // W-tagging studies
        ...
        Float_t recoW_Tau1[10];
        Float_t recoW_Tau2[10];
        Float_t recoW_Tau3[10];
        Float_t recoW_pTsj1[10];
        Float_t recoW_pTsj2[10];
        Float_t recoW_Msj1[10];
        Float_t recoW_Msj2[10];
...
//                       #            Keep me updated !             #
//                       ############################################
#define MICROEVENT_FORMATROOT "trueNumberOfEvents:nJets:nBtags:jet_pT[10]:jet_eta[10]:jet_phi[10]:jet_bTag[10]:lep_pT:lep_eta:lep_phi:lep_flavor:MET:MT:HadronicChi2:MT2W:DPhi_MetJets:isolatedTrackVeto:tauVeto:mStop:mNeutralino:genW_pT:genW_Eta:genW_Phi:recoW_pT[10]:recoW_Eta[10]:recoW_Phi[10]:recoW_Mass[10]:recoW_Tau1[10]:recoW_Tau2[10]:recoW_Tau3[10]:recoW_pTsj1[10]:recoW_pTsj2[10]:recoW_Msj1[10]:recoW_Msj2[10]"
  ...
  // ######################
    ...
    // Loop on W-candidate collection
        ...
        for (unsigned int i = 0 ; i < WCand.size() ; i++)
        {
            ...
            //myEvent.recoW_Tau1[i] = WCand[i].others["tau1"];
            //myEvent.recoW_Tau2[i] = WCand[i].others["tau2"];
            //myEvent.recoW_Tau3[i] = WCand[i].others["tau3"];
            myEvent.recoW_Tau1[i] = -1; 
            myEvent.recoW_Tau2[i] = -1;
            myEvent.recoW_Tau3[i] = -1;

            if (WCand[i].subjets.size() >= 1)
            {   myEvent.recoW_pTsj1[i] = WCand[i].subjets[0].p4.Pt();
                myEvent.recoW_Msj1[i] = WCand[i].subjets[0].p4.M();     }
            else
            {   myEvent.recoW_pTsj1[i] = -1;
                myEvent.recoW_Msj1[i]  = -1;                            }

            if (WCand[i].subjets.size() >= 2)
            {   myEvent.recoW_pTsj2[i] = WCand[i].subjets[1].p4.Pt();
                myEvent.recoW_Msj2[i]  = WCand[i].subjets[1].p4.M();    }
            else
            {   myEvent.recoW_pTsj2[i] = -1;
                myEvent.recoW_Msj2[i]  = -1;                            }
        }
